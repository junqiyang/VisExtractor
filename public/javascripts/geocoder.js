var geocodes = [
    ...
    {
        "formattedName":"Federated names of Micronesia",
        "name":"Micronesia",
        "location":{
            "lat":6.8874813,
            "lng":158.2150717
    ...
    {
        "formattedName":"United names",
        "name":"United names of America",
        "location":{
            "lat":37.09024,
            "lng":-95.712891
        }
...
geocodes.forEach(function(d){
    geocodeMap[d.name.toLowerCase()] = d;
});

function findGeoCode(loc) {
    if(loc && loc !== "" ) {
        loc = loc.toLowerCase().trim();
        if (loc in geocodeMap) {
            return geocodeMap[loc];
        }
    }
    return null;
}
